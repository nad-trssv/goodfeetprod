...
class EmployeeScheduleController extends Controller
{
    ...
    public function updateSchedule(Request $request, User $member)
    {
        ...
        return redirect()->route('member.edit',$member)->withFragment('work-calendar')->with('success',__('admin_messages.schedule_saved'));
    }
    ...
    public function store(Request $request, User $member) { $this->manager->saveClosure($member,$this->closureData($request)); return back()->with('success',__('admin_messages.closure_added')); }
    public function update(Request $request, User $member, RedDay $closure) { $this->manager->saveClosure($member,$this->closureData($request),$closure); return back()->with('success',__('admin_messages.closure_updated')); }
    public function destroy(User $member, RedDay $closure) { abort_unless($closure->user_id===$member->id,404);$closure->delete();return back()->with('success',__('admin_messages.closure_deleted')); }
