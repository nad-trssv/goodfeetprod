@section('title', __('admin_schedule.my_closed'))

<x-dashboard-layout>
    <div class="content">
        <nav class="mb-3" aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item active">{{ __('admin_schedule.my_closed') }}</li>
            </ol>
        </nav>
        ...
        <div class="row mb-4 align-items-center">
            <div class="col">
                <h2 class="mb-0">{{ __('admin_schedule.my_closed') }}</h2>
                <p class="text-muted fs-9 mt-1">{{ __('admin_schedule.my_closed_hint') }}</p>
            </div>
            <div class="col-auto">
                ...
                <form method="POST" action="{{ route('master.time-off.store') }}">
                        ...
                        <div class="col-md-3 d-flex align-items-end">
                            <div class="form-check mb-1">
                                <input class="form-check-input" type="checkbox" name="repeat" id="new_repeat"
                                    value="1">
                                <label class="form-check-label" for="new_repeat">{{ __('admin_schedule.repeat_yearly') }}</label>
                            </div>
                        </div>
                    </div>
                    <div class="row g-3 mt-1" id="new_time_block" style="display:none">
                        ...
                        <div class="col-md-8 d-flex align-items-end">
                            <p class="text-muted fs-10 mb-1"><span class="fas fa-info-circle me-1"></span>{{ __('admin_schedule.specify_time') }}</p>
                        </div>
                    </div>
                ...
                <form method="GET" action="{{ route('master.time-off.index') }}" class="row g-3 mb-4">
                    ...
                    <div class="col-md-2">
                        <select class="form-select form-select-sm" name="type">
                            ...
                            <option value="full" {{ request('type') == 'full' ? 'selected' : '' }}>{{ __('admin_schedule.full_day') }}</option>
                    ...
                    <div class="col-md-2">
                        <select class="form-select form-select-sm" name="owner">
                            <option value="">{{ __('admin_schedule.all') }}</option>
                            <option value="mine" {{ request('owner') == 'mine' ? 'selected' : '' }}>{{ __('admin_schedule.mine') }}</option>
                            <option value="common" {{ request('owner') == 'common' ? 'selected' : '' }}>{{ __('admin_schedule.common') }}</option>
                        </select>
                    </div>
                    <div class="col-md-2">
                    ...
                    <table class="table table-sm fs-9 mb-0">
                        ...
                        <tbody>
                            @forelse($filtered as $day)
                                <tr>
                                    <td>{{ $day->name }}</td>
                                    <td>{{ \Carbon\Carbon::parse($day->date)->format('d.m.Y') }}@if($day->endDate()->format('Y-m-d') !== \Carbon\Carbon::parse($day->date)->format('Y-m-d')) – {{ $day->endDate()->format('d.m.Y') }}@endif<br><small class="text-muted">{{ $day->typeLabel() }}</small></td>
                                    <td>
                                        @if ($day->full_day)
                                    ...
                                    <td>
                                        @if ($day->user_id)
                                            <span class="badge badge-phoenix badge-phoenix-primary fs-10">{{ __('admin_schedule.mine_badge') }}</span>
                                        @else
                                            <span
                                    ...
                                    <td class="text-end">
                                        @if ($day->user_id || auth()->user()->hasAllAppointmentsScope())
                                            <button class="btn btn-sm btn-outline-primary me-1" type="button"
                                                ...
                                                data-date-to="{{ $day->endDate() }}" data-type="{{ $day->type ?: 'other' }}"
