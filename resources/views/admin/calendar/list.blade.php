@section('title', __('admin_appointments.appointments'))
<x-dashboard-layout>
  <div class="content">
    <nav class="mb-3" aria-label="breadcrumb">
      <ol class="breadcrumb mb-0">
        <li class="breadcrumb-item active">{{ __('admin_appointments.appointments') }}</li>
      </ol>
    </nav>
        ...
        <div class="col col-auto">
          <div class="search-box">
            <form class="position-relative">
              <input class="form-control search-input search" type="search" placeholder="{{ __('admin_appointments.search') }}" aria-label="{{ __('admin_staff.find') }}" />
      ...
      <div class="mx-n4 mx-lg-n6 px-4 px-lg-6 mb-9 bg-body-emphasis border-y mt-2 position-relative top-1">
          ...
          <table class="table table-sm fs-9 mb-0">
            <thead>
              <tr>
                ...
                <th class="sort align-middle text-uppercase" scope="col" data-sort="membername" style="width:30%; min-width:200px;">{{ __('admin_appointments.service_employee') }}</th>
                <th class="sort align-middle text-uppercase" scope="col" data-sort="client" style="width:20%; min-width:200px;">{{ __('admin_appointments.client') }}</th>
                <th class="sort align-middle text-uppercase" scope="col" data-sort="role" style="width:5%;">{{ __('admin_appointments.notes') }}</th>
                <th class="sort align-middle text-end pe-0 text-uppercase" scope="col" data-sort="joined" style="width:29%; min-width:200px;">{{ __('admin_appointments.appointment_time') }}</th>
                ...
                <td class="membername align-middle">
                  <a class="d-flex align-items-center text-body text-hover-1000 no-underline" href="{{ route('calendar.show', ['appointment' => $appint['id']]) }}">
                    <div class="flex-col ms-3">
                      ...
                      <div class="fs-9 fw-bold w-auto">{{ __('admin_appointments.employee') }}: {{ $appint['master'] }}</div>
                      ...
                      <div class="fs-10 fw-bold w-auto">
                        @if ($appint['appointment_time_from'])
                          ( {{ $appint['appointment_time_from'] }} - {{ __('admin_appointments.minutes',['count'=>$appint['appointment_time_to']]) }} )
                        @else
                          ( {{ __('admin_appointments.minutes',['count'=>$appint['appointment_time_to']]) }} )
                        @endif
                      </div>
                    </div>
                  </a>
        ...
        <div class="row align-items-center justify-content-between py-2 pe-0 fs-9">
          <div class="col-auto d-flex">
            ...
            <a class="fw-semibold" href="#!" data-list-view="*">{{ __('admin_appointments.view_all') }} <span class="fas fa-angle-right ms-1"></span></a>
            <a class="fw-semibold d-none" href="#!" data-list-view="less">{{ __('admin_appointments.view_less') }} <span class="fas fa-angle-right ms-1"></span></a>
          </div>
          <div class="col-auto d-flex">
