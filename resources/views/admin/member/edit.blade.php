    ...
    <section class="tab-pane fade show active" id="main">
        ...
        <div class="row g-4">
          <div class="col-12 col-xl-8">
            ...
            <div class="row g-3">
              ...
              <div class="col-12 col-md-6"><label class="form-label" for="role_id">Роль</label><select class="form-select" id="role_id">@foreach($roles as $role)<option value="{{ $role->id }}" @selected($member->role_id === $role->id)>{{ $role->name }}</option>@endforeach</select></div>
          ...
          <div class="col-12 col-xl-4">
            ...
            <button class="btn btn-primary w-100" id="saveBtn" type="button"><span class="fas fa-save me-1"></span>Сохранить основное</button>
          </div>
        </div>
      </div></div>
      ...
      <div class="card mt-4"><div class="card-body p-4"><h4 class="mb-1">Получатели уведомлений</h4><p class="text-body-tertiary">Кто получает административные уведомления о записях этого сотрудника.</p>
        <form method="POST" action="{{ route('member.notification-recipients.update', $member) }}">@csrf
          ...
          <div class="row g-2">@foreach($admins as $admin)<div class="col-12 col-lg-6"><label class="border rounded p-3 d-flex align-items-center gap-2 w-100"><input class="form-check-input mt-0" type="checkbox" name="recipients[]" value="{{ $admin->id }}" @checked(in_array($admin->id, $recipientIds)) @disabled($admin->id === $member->id)><img src="{{ $admin->profile_photo_url }}" class="rounded-circle" style="width:32px;height:32px;object-fit:cover"><span class="min-w-0"><strong class="d-block text-break">{{ $admin->name }}</strong><small class="text-body-tertiary text-break">{{ $admin->email }}</small></span></label></div>@endforeach</div>
          <input type="hidden" name="recipients[]" value="{{ $member->id }}"><button class="btn btn-outline-primary mt-3" type="submit">Сохранить получателей</button>
        </form>
      </div></div>
    </section>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('saveBtn')?.addEventListener('click', async function(){
    ...
    ['name','username','email','phone','role_id','date_birthday','employment_started_at'].forEach(id => data.append(id, document.getElementById(id).value));
