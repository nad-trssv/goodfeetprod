@section('title', __('admin_staff.employees'))
@push('styles')
<style>
...
<x-dashboard-layout>
  <div class="content">
    <nav class="mb-3" aria-label="breadcrumb"><ol class="breadcrumb mb-0"><li class="breadcrumb-item active">{{ __('admin_staff.employees') }}</li></ol></nav>
    ...
    <div class="team-summary mb-4">
      <div class="team-summary__item"><div class="team-summary__value">{{ $summary['total'] }}</div><small class="text-body-tertiary">{{ __('admin_staff.total') }}</small></div>
      <div class="team-summary__item"><div class="team-summary__value text-success">{{ $summary['online'] }}</div><small class="text-body-tertiary">{{ __('admin_staff.online') }}</small></div>
      <div class="team-summary__item"><div class="team-summary__value text-success">{{ $summary['available'] }}</div><small class="text-body-tertiary">{{ __('admin_staff.available_now') }}</small></div>
      <div class="team-summary__item"><div class="team-summary__value text-warning">{{ $summary['busy'] }}</div><small class="text-body-tertiary">{{ __('admin_staff.with_client') }}</small></div>
      <div class="team-summary__item"><div class="team-summary__value text-body-tertiary">{{ $summary['not_working'] }}</div><small class="text-body-tertiary">{{ __('admin_staff.not_working') }}</small></div>
    </div>

    <div class="team-toolbar d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
      <div class="search-box flex-grow-1" style="max-width:420px"><form class="position-relative" onsubmit="return false"><input id="memberSearch" class="form-control search-input" type="search" placeholder="{{ __('admin_staff.search_placeholder') }}"><span class="fas fa-search search-box-icon"></span></form></div>
      <div class="btn-group" role="group" aria-label="{{ __('admin_staff.employee_filter') }}">
        <button class="btn btn-outline-secondary filter-btn active" data-filter="all">{{ __('admin_staff.all') }}</button>
        <button class="btn btn-outline-secondary filter-btn" data-filter="online">{{ __('admin_staff.online') }}</button>
        <button class="btn btn-outline-secondary filter-btn" data-filter="available">{{ __('admin_staff.available') }}</button>
        <button class="btn btn-outline-secondary filter-btn" data-filter="busy">{{ __('admin_staff.busy') }}</button>
        <button class="btn btn-outline-secondary filter-btn" data-filter="not_working">{{ __('admin_staff.not_working') }}</button>
      </div>
    </div>
      ...
      @foreach($members as $user)
        ...
        <article class="member-card" data-member-card data-state="{{ $state['code'] }}" data-online="{{ $user['is_online'] ? '1' : '0' }}" data-search="{{ mb_strtolower($user['name'].' '.$user['email'].' '.$user['phone'].' '.$user['username']) }}">
          ...
          <div class="member-meta mb-3">
            <div class="member-meta__item"><small>{{ __('admin_staff.today') }}</small><strong>{{ $user['today_hours'] ?: __('admin_staff.day_off') }}</strong></div>
            <div class="member-meta__item"><small>{{ __('admin_staff.activity') }}</small><strong>{{ $user['is_online'] ? __('admin_staff.online_now') : $user['last_seen'] }}</strong></div>
            <div class="member-meta__item"><small>{{ __('admin_staff.services') }}</small><strong>{{ __('admin_staff.active_count',['count'=>$user['services_count']]) }}</strong></div>
            <div class="member-meta__item"><small>{{ __('admin_staff.status') }}</small><strong title="{{ $state['detail'] }}">{{ $state['detail'] ?: '—' }}</strong></div>
          </div>

          <div class="member-outcomes mb-3">
            <div title="{{ __('admin_staff.completed') }}"><strong class="text-success">{{ $user['today_stats']['completed'] }}</strong><small>{{ __('admin_staff.done') }}</small></div>
            <div title="{{ __('admin_staff.revenue') }}"><strong class="text-success">{{ number_format($user['today_stats']['earned'],0,',',' ') }} €</strong><small>{{ mb_strtolower(__('admin_staff.revenue')) }}</small></div>
            <div title="{{ __('admin_staff.clients_no_show') }}"><strong class="text-danger">{{ $user['today_stats']['no_show'] }}</strong><small>{{ mb_strtolower(__('admin_staff.no_shows')) }}</small></div>
            <div title="{{ __('admin_staff.lost_no_shows') }}"><strong class="text-danger">{{ number_format($user['today_stats']['lost'],0,',',' ') }} €</strong><small>{{ __('admin_staff.lost') }}</small></div>
            <div title="{{ __('admin_staff.cancelled') }}"><strong>{{ $user['today_stats']['cancelled'] }}</strong><small>{{ mb_strtolower(__('admin_staff.cancelled')) }}</small></div>
          </div>
          ...
          @if($user['action_required'] && (auth()->user()->hasAllAppointmentsScope() || auth()->id() === $user['id']))
            <div class="appointment-panel appointment-panel--action mb-3" data-status-panel>
              <small>{{ __('admin_staff.result_required',['time'=>$user['action_required']['start']]) }}</small><strong>{{ $user['action_required']['service'] }}</strong><span class="fs-10 text-body-secondary mb-2">{{ $user['action_required']['client'] }}</span>
              <div class="d-flex gap-2"><button class="btn btn-sm btn-success flex-grow-1 appointment-status" data-id="{{ $user['action_required']['id'] }}" data-status="completed">{{ __('admin_staff.completed') }}</button><button class="btn btn-sm btn-outline-danger flex-grow-1 appointment-status" data-id="{{ $user['action_required']['id'] }}" data-status="no_show">{{ __('admin_staff.no_show') }}</button></div>
            </div>
          @elseif($user['current_appointment'])
            <div class="appointment-panel mb-3"><small>{{ __('admin_staff.current_appointment',['time'=>$user['current_appointment']['end']]) }}</small><strong>{{ $user['current_appointment']['service'] }}</strong><span class="fs-10 text-body-secondary">{{ $user['current_appointment']['client'] }}</span></div>
          @elseif($user['next_appointment'])
            <div class="appointment-panel mb-3"><small>{{ __('admin_staff.next_appointment_at',['time'=>$user['next_appointment']['start']]) }}</small><strong>{{ $user['next_appointment']['service'] }}</strong><span class="fs-10 text-body-secondary">{{ $user['next_appointment']['client'] }}</span></div>
          @else
