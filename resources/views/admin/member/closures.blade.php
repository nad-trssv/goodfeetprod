@section('title', 'Календарь исключений — '.$member->name)
@push('styles')
...
<x-dashboard-layout>
<div class="content">
  <nav class="mb-3"><ol class="breadcrumb"><li class="breadcrumb-item"><a href="{{ route('member.index') }}">Сотрудники</a></li><li class="breadcrumb-item"><a href="{{ route('member.edit',$member) }}#work-calendar">{{ $member->name }}</a></li><li class="breadcrumb-item active">Исключения</li></ol></nav>
  @if(session('success'))<div class="alert alert-success">{{ session('success') }}</div>@endif
  @if($errors->any())<div class="alert alert-danger">{{ $errors->first() }}</div>@endif
  <div class="d-flex flex-column flex-md-row justify-content-between gap-3 mb-4"><div><h2>Календарь исключений</h2><p class="text-body-tertiary mb-0">{{ $member->name }}: красным показаны личные исключения, серым — общие нерабочие дни компании.</p></div><button class="btn btn-primary align-self-start" data-bs-toggle="modal" data-bs-target="#createClosure">Добавить исключение</button></div>
  <div class="card"><div class="card-body p-2 p-md-4"><div id="closure-calendar"></div></div></div>

  <div class="modal fade" id="createClosure" tabindex="-1"><div class="modal-dialog"><form class="modal-content" method="POST" action="{{ route('member.closures.store',$member) }}">@csrf<div class="modal-header"><h5 class="modal-title">Новое исключение</h5><button class="btn-close" type="button" data-bs-dismiss="modal"></button></div><div class="modal-body">@include('admin.member.partials.closure-form',['prefix'=>'create'])</div><div class="modal-footer"><button class="btn btn-secondary" type="button" data-bs-dismiss="modal">Отмена</button><button class="btn btn-primary">Сохранить</button></div></form></div></div>
  <div class="modal fade" id="editClosure" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><form id="editClosureForm" method="POST">@csrf @method('PUT')<div class="modal-header"><h5 class="modal-title">Редактировать исключение</h5><button class="btn-close" type="button" data-bs-dismiss="modal"></button></div><div class="modal-body">@include('admin.member.partials.closure-form',['prefix'=>'edit'])</div><div class="modal-footer"><button class="btn btn-secondary" type="button" data-bs-dismiss="modal">Отмена</button><button class="btn btn-primary">Сохранить</button></div></form><form id="deleteClosureForm" method="POST" class="px-3 pb-3" onsubmit="return confirm('Удалить исключение?')">@csrf @method('DELETE')<button class="btn btn-outline-danger w-100">Удалить</button></form></div></div></div>
  <x-dashboard-footer />
</div>
@push('scripts')
