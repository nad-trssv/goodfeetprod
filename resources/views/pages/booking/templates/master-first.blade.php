    ...
    <div class="row g-4">
      <aside class="col-12 col-lg-4 col-xl-3"><div class="master-directory"><h2 class="fs-6 mb-3">{{ __('msg.choose_master') }}</h2><div id="altMasters">
        @foreach($bookingMasters as $master)
          <button type="button" class="alt-master master-directory__item" data-master-id="{{ $master['id'] }}"><span class="master-directory__avatar">{{ mb_strtoupper(mb_substr($master['name'], 0, 1)) }}</span><span><strong>{{ $master['name'] }}</strong><small>{{ __('msg.booking_view_services') }}</small></span><i class="fa-solid fa-chevron-right"></i></button>
        @endforeach
      </div></div></aside>
