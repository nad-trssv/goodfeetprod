(function (factory) {
  ...
  // Function to fetch data from Laravel API and merge it into the `events` array
  async function fetchEventsFromAPI() {
    try {
      const response = await fetch('http://127.0.0.1:8000/api/appointments'); // Replace with your actual Laravel API URL
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const apiEvents = await response.json();
  
      // Merge API events with existing events
      apiEvents.forEach(event => {
        events.push({
          title: event.title,
          start: event.start,
          end: event.end || null, // Optional property
      ...
      console.log('Updated Events:', events); // For debugging
    } catch (error) {
      console.error('Failed to fetch events from API:', error);
    }
  }
  
  // Call the function to fetch and update the events array
  fetchEventsFromAPI();
